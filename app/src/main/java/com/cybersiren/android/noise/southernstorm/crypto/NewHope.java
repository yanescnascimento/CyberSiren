...
public class NewHope {
	...
	public void keygen(byte[] send, int sendOffset)
	{
	  ...
	  try {
		  randombytes(seed);
		  sha3256(seed, 0, seed, 0, SEEDBYTES);
	...
	private class Poly
	{
		...
		public void tobytes(byte[] r, int offset)
		{
			...
			for (i = 0; i < PARAM_N/4; i++)
			{
			    t0 = barrett_reduce(coeffs[4*i+0]);
		...
		public void pointwise(Poly a, Poly b)
		{
		  ...
		  for(i=0;i<PARAM_N;i++)
		  {
		    ...
		    coeffs[i] = (char)montgomery_reduce(a.coeffs[i] * t);
		  }
		}
	...
	private static final int qinv = 12287;
	...
	private static final int bitrev_table_combined[] = {
	...
	private static final char[] omegas_montgomery = {
	...
	private static final char[] omegas_inv_montgomery = {
	...
	private static final char[] psis_bitrev_montgomery = {
		4075,6974,7373,7965,3262,5079,522,2169,6364,1018,1041,8775,2344,
		11011,5574,1973,4536,1050,6844,3860,3818,6118,2683,1190,4789,7822,
	...
	private static final char[] psis_inv_montgomery = {
