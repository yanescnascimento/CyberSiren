        ...
        if (isPresented) {
            ...
            while (true) {
                ...
                val devices = entries.map { (address, isClient, rssi) ->
                    ...
                    com.cybersiren.android.ui.debug.ConnectedDevice(
                ...
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            if (graphMode != GraphMode.OVERALL && stackedKeysIncoming.isNotEmpty()) {  }
                            ...
                            if (graphMode != GraphMode.OVERALL && stackedKeysOutgoing.isNotEmpty()) {  }
                        }
                    }
                }
            }
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        val p = remember(gcsFpr) { com.cybersiren.android.sync.GCSFilter.deriveP(gcsFpr / 100.0) }
                        val nmax = remember(gcsFpr, gcsMaxBytes) { com.cybersiren.android.sync.GCSFilter.estimateMaxElementsForSize(gcsMaxBytes, p) }
